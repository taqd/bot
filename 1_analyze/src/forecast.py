...
def write_stat(ddir, filename, suffix, value, prediction):
...
#        write_stat(fdir, filename, "arima012",     fcast.values[-1])
        ...
        fcast = sm.tsa.arima.ARIMA(series, order=(2, 0, 1)).fit().forecast(steps=1);
        write_stat(fdir, filename, "arima201", series[0], fcast[-1])
...
#        write_stat(fdir, filename, "arima120",     fcast.values[-1])
        ...
        res = mod_dfm = sm.tsa.DynamicFactorMQ(series, k_factors=1, factor_order=2).fit();
        fcast = res.forecast(steps=1)
        write_stat(fdir, filename, "dynamicfmq21", series[0], fcast[-1])
...
#        write_stat(fdir, filename, "arima012",     0)  
        write_stat(fdir, filename, "arima201",     0,0)  
#        write_stat(fdir, filename, "arima120",     0)  
#        write_stat(fdir, filename, "dynamicfmq12", 0,0)  
        write_stat(fdir, filename, "dynamicfmq21", 0,0)  
        continue 
